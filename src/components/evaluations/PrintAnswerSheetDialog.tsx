    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Configura la generación de filas (versiones) para la evaluación. Se creará una hoja de respuesta para cada estudiante.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onConfirm)} className="space-y-4 py-4">
          ...
          <div>
            <Label htmlFor="seed">Semilla (Palabra Clave)</Label>
            <Controller
              name="seed"
              control={control}
              render={({ field }) => <Input id="seed" {...field} />}
            />
            <p className="text-xs text-muted-foreground mt-1">
              Usa la misma palabra clave para generar siempre las mismas filas.
            </p>
